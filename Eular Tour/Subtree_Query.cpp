...
vector<int>node[N];
ll Intime[N], Outtime[N], Level[N] , a[N];
int timer = 1;
int n, q;
...
void Solve()
{
    ...
    // All subtree of x are in range Intime[x]....Outime[x]-1;
    

    for (int i = 1 ; i <= n ; i++)
    {
        T.upd(Intime[i], Intime[i], a[i]);
    }
    ...
    while (q--)
    {
        ...
        if (type == 1)
        {
            ll x, val;
            cin >> x >> val;
            T.upd(Intime[x], Intime[x], -T.query(Intime[x], Intime[x]));
            T.upd(Intime[x], Intime[x], val);
        }
        else
        {
            ll x;
            cin >> x;
            cout << T.query(Intime[x], Outtime[x]-1) << endl;
        }
